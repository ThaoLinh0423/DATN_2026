"""
Multi-feature stacked Bidirectional LSTM with direct multi-step forecasting.
Fixed: Single-target scaling and multi-step output (HORIZON) to match retrain.py logic.
...
class _BiLSTMModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, dropout=0.2, horizon=1, num_targets=1):
        super().__init__()
        ...
        for hidden in hidden_sizes:
            ...
            self.layer_norms.append(nn.LayerNorm(hidden * 2))
            in_size = hidden * 2
    ...
    def forward(self, x):
        out = x
        for i, (lstm, drop, norm) in enumerate(
                zip(self.bilstm_layers, self.dropouts, self.layer_norms)):
            out, _ = lstm(out)
            out    = norm(out)
            out    = drop(out)
...
class BiLSTM(BaseTimeSeriesModel):
    ...
    def _resolve_window_params(self, train_len, test_len):
        ...
        if train_len - look_back - horizon + 1 < 1:
            raise ValueError(
                f"BiLSTM requires at least LOOK_BACK + HORIZON samples in train split. "
                f"Configured LOOK_BACK={look_back}, HORIZON={horizon}, train_len={train_len}."
            )

        if test_len < horizon:
            raise ValueError(
                f"BiLSTM requires at least HORIZON samples in test split for backtest. "
    ...
    def train(self, train_data, df_model=None):
        ...
        hidden_sizes = getattr(config, "BILSTM_HIDDEN_SIZES", [128, 64, 32])
        dropout      = getattr(config, "BILSTM_DROPOUT", 0.2)
    ...
    def save(self, out_dir=None, name=None):
        ...
        torch.save(self.model.state_dict(), base + "_bilstm.pt")
        joblib.dump(self.scaler_X, base + "_bilstm_scaler_X.joblib")
        joblib.dump(self.scaler_y, base + "_bilstm_scaler_y.joblib")
        joblib.dump(
            {
    ...
    def load(cls, out_dir=None, name=None):
        ...
        name    = name or "bilstm"
        base    = str(out_dir / name)
        state_dict = torch.load(base + "_bilstm.pt", map_location=DEVICE)
        scaler_X = joblib.load(base + "_bilstm_scaler_X.joblib")
        scaler_y = joblib.load(base + "_bilstm_scaler_y.joblib")

        meta_path = base + "_bilstm_meta.joblib"
        meta = joblib.load(meta_path) if os.path.exists(meta_path) else {}

        layer_ids = {
            int(match.group(1))
            for key in state_dict
            if (match := re.match(r"bilstm_layers\.(\d+)\.weight_ih_l0$", key))
        }
        ordered_ids = sorted(layer_ids)
        hidden_sizes = [
            int(state_dict[f"bilstm_layers.{layer_id}.weight_ih_l0"].shape[0] // 4)
            for layer_id in ordered_ids
        ]
        if not hidden_sizes:
            hidden_sizes = list(meta.get("hidden_sizes", getattr(config, "BILSTM_HIDDEN_SIZES", [128, 64, 32])))
        input_size = int(state_dict[f"bilstm_layers.{ordered_ids[0]}.weight_ih_l0"].shape[1]) if ordered_ids else getattr(scaler_X, "n_features_in_", len(config.FEATURES))
