"""
Multi-feature stacked LSTM with direct multi-step forecasting.
PyTorch implementation — architecture from lstm_aqi_pytorch.py.
Fixed: IndexError by outputting full HORIZON steps and proper inverse scaling.
...
class _StackedLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.2, horizon=1, num_targets=1):
        super().__init__()
    ...
    def forward(self, x):
        out, _ = self.lstm(x)
        out = out[:, -1, :]  # Lay dac trung tu timestep cuoi cung
...
class LSTM(BaseTimeSeriesModel):
    ...
    def _prepare_df(self, df_model):
        ...
        return (df_model[inp_feats].values, df_model[feat_existing].values, inp_feats, feat_existing)

    def _make_supervised(self, arr_inp, arr_tgt_multi):
        X, Y = [], []
        # Tao output cho nhieu targets: moi sample se co horizon x num_targets outputs
        for i in range(self.look_back, len(arr_inp) - self.horizon + 1):
            X.append(arr_inp[i - self.look_back: i, :])
            # Lay tat ca cac target columns cho horizon steps
    ...
    def _resolve_window_params(self, train_len, test_len):
        ...
        if train_len - look_back - horizon + 1 < 1:
            raise ValueError(
                f"LSTM requires at least LOOK_BACK + HORIZON samples in train split. "
                f"Configured LOOK_BACK={look_back}, HORIZON={horizon}, train_len={train_len}."
            )

        if test_len < horizon:
            raise ValueError(
                f"LSTM requires at least HORIZON samples in test split for backtest. "
    ...
    def train(self, train_data, df_model=None):
        ...
        if df_model is not None:
            ...
            # Chon tat ca cac target columns
        ...
        train_loader = DataLoader(_AQIDataset(X_train, Y_train), batch_size=config.BATCH_SIZE, shuffle=True)
        val_loader   = DataLoader(_AQIDataset(X_val, Y_val), batch_size=config.BATCH_SIZE, shuffle=False)

        num_targets = len(self.target_columns)
        self.model = _StackedLSTM(input_size=arr_inp.shape[1],
                                 hidden_size=getattr(config, "LSTM_HIDDEN_SIZE", 200),
                                 num_layers=getattr(config, "LSTM_NUM_LAYERS", 4),
        ...
        for epoch in range(1, config.EPOCHS + 1):
            ...
            with torch.no_grad():
                for xb, yb in val_loader:
                    xb, yb = xb.to(DEVICE), yb.to(DEVICE)
                    v_loss += criterion(self.model(xb), yb).item() * len(xb)
    ...
    def predict(self, train_data, test_data, df_model=None):
        ...
        num_targets = len(self.target_columns)
        preds_inv = self.scaler_y.inverse_transform(preds.reshape(-1, num_targets)).reshape(preds.shape)
        trues_inv = self.scaler_y.inverse_transform(trues.reshape(-1, num_targets)).reshape(trues.shape)
    ...
    def _forecast_direct(self, last_window_scaled, last_ts):
        ...
        num_targets = len(self.target_columns)
        pred_inv = self.scaler_y.inverse_transform(pred_s[0].reshape(-1, num_targets))
    ...
    def _forecast_iterative(self, last_window_scaled, last_ts):
        ...
        for _ in range(self.horizon):
            ...
            with torch.no_grad():
                next_s = self.model(X).cpu().numpy()[0, 0, :]  # Lay target dau tien cho buoc tiep theo

            step_pred = self.scaler_y.inverse_transform(next_s.reshape(1, -1)).flatten()
            rows.append(step_pred)

            # Cap nhat window - chi update target columns
    ...
    def save(self, out_dir=None, name=None):
        ...
        torch.save(self.model.state_dict(), base + "_lstm.pt")
        joblib.dump(self.scaler_X, base + "_lstm_scaler_X.joblib")
        joblib.dump(self.scaler_y, base + "_lstm_scaler_y.joblib")
        joblib.dump(
            {
                ...
                "horizon": self.horizon,
                "hidden_size": getattr(self.model.lstm, "hidden_size", getattr(config, "LSTM_HIDDEN_SIZE", 200)),
                "num_layers": getattr(self.model.lstm, "num_layers", getattr(config, "LSTM_NUM_LAYERS", 4)),
                "num_targets": len(self.target_columns),
            },
            base + "_lstm_meta.joblib",
        )
    ...
    def load(cls, out_dir=None, name=None):
        ...
        name    = name or config.MODEL_NAME
        base    = str(out_dir / name)
        state_dict = torch.load(base + "_lstm.pt", map_location=DEVICE)
        scaler_X = joblib.load(base + "_lstm_scaler_X.joblib")
        scaler_y = joblib.load(base + "_lstm_scaler_y.joblib")

        meta_path = base + "_lstm_meta.joblib"
        meta = joblib.load(meta_path) if os.path.exists(meta_path) else {}

        input_size = int(state_dict["lstm.weight_ih_l0"].shape[1])
        hidden_size = int(meta.get("hidden_size", state_dict["lstm.weight_ih_l0"].shape[0] // 4))
        layer_ids = {
            int(match.group(1))
            for key in state_dict
            if (match := re.match(r"lstm\.weight_ih_l(\d+)$", key))
        }
        num_layers = int(meta.get("num_layers", (max(layer_ids) + 1) if layer_ids else getattr(config, "LSTM_NUM_LAYERS", 4)))
